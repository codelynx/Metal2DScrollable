...
/*
	func render(context: RenderContext, vertices: [Vertex]) {
		let length = MemoryLayout<Vertex>.size * vertices.count
		let vertexBuffer = self.device.makeBuffer(bytes: vertices, length: length, options: [])
		vertexBuffer.label = "colors"
